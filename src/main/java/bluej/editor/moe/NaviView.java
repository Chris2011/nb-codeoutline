...
/**
 * "NaviView" component. Displays a miniature version of the document in the editor, and allows moving through the document by clicking/dragging or cycling the mouse wheel.
 *
 * @author Davin Mccall
 */
public class NaviView extends JPanel implements AdjustmentListener {
	...
	InputOutput io = IOProvider.getDefault().getIO("outline ar", false);
	...
	/**
	 * Notify the NaviView that the document has changed (vertical) length. Normally this is called by NaviviewView.
	 */
	...
	/**
	 * Repaint model co-ordinates. The given lines will be translated to the NaviView component's co-ordinate space.
	 *
...
//        haveToolTip = false;
	...
	/**
	 * Paint to the backing image buffer, and then issue an appropriate repaint() so that the backing buffer is displayed.
	 *
...
//			view.paint(g, bufferBounds);
	...
	private void makeImage(BufferedImage image, String file) {
		try {
			ImageIO.write(image, "png", new File("/Users/peter/Desktop/" + file));
	...
	private void makePanelImage(Component panel) {
		...
		panel.paint(g2);
		try {
			ImageIO.write(image, "png", new File("/Users/peter/Desktop/fuck_imgBuffer_original.png"));
			System.out.println("Panel saved as Image.");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * Enqueue a repaint of the backing buffer. The given co-ordinates (top, bottom) are view co-ordinates.
	 */
